...
export const contentJa = {
  ...
  about: {
    title: 'Flyingnets — AIとセキュリティ駆動のグローバル企業ソリューション',
    content: `Flyingnetsはシンガポールに本社を置く、アジア太平洋地域を代表するAI・セキュリティ統合ソリューションプロバイダーです。AI技術で企業効率を向上させ、セキュリティ技術で企業情報を保護し、グローバル企業のインテリジェント変革を支援しています。

Flyingnetsは日本、香港、上海、成都、北京、広州の6つの地域オフィスを展開し、アジア太平洋をカバーするグローバルサービスネットワークを構築。製品開発、サービス提供、運用の全プロセスにAIを深く統合し、セキュリティ運用センター（SOC）から企業向けAI従業員プラットフォームまで、自社開発のSynergy AI、ALL-SOC、ASSAなどの製品群で、金融、製造、コンサルティング、小売など各業界のお客様に実践可能なインテリジェント変革パスを提供しています。
...
Flyingnetsは、真のインテリジェント運用とはAIをビジネスの一部にすることであり、セキュリティが企業データの一つひとつを守ることだと信じています。`,
  },
  services: [
    {
      title: 'クラウドサービス',
